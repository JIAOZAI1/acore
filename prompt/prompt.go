...
// Renderer generates one complete system prompt. Implementations must be safe
// for concurrent calls and must not retain Input or its Values.
...
// Render calls f after validating ctx. A context error observed after f returns
// takes precedence over f's result.
