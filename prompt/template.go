...
// Template is an immutable strict text template. Missing variables fail
// rendering instead of producing a zero value.
type Template struct {
	parsed   *template.Template
	defaults Values
}

// NewTemplate parses config and snapshots its default values.
func NewTemplate(config TemplateConfig) (*Template, error) {
	...
	return &Template{
		parsed:   parsed,
		defaults: cloneValues(config.Defaults),
	}, nil
}

// Render applies input values over the defaults and executes the template.
func (t *Template) Render(ctx context.Context, input Input) (string, error) {
	...
	err := t.parsed.Execute(&output, values)
